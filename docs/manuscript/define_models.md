...
デザイン行列 $\mathbf{X}$ は以下のように定義される．

$$
\mathbf{X} =
\begin{bmatrix}
    \text{island}_1 & \text{year}_1 & W_1 \\
    \vdots & \vdots & \vdots \\
    \text{island}_n & \text{year}_n & W_n \\
\end{bmatrix}
$$

添字は観測値を表し，$\text{island}$ は島ダミー，$\text{year}$ は年ダミー，$W$ は介入変数である．

ハット行列 $\mathbf{H}$ は以下のように計算する．
...
$$
\text{L}_{i} = \text{diag}(\mathbf{H})_i
$$

添字 $i$ は観測番号を表し，$\text{diag}(\mathbf{H})$ はハット行列の対角成分を取り出したベクトルを表す．

以上のように計算されたレバレッジは $0$ から $1$ の値を取り，観測値がデータ全体に与える影響の大きさを示す．
...
このような外れ値が存在する場合，正規分布を仮定したモデルは外れ値に対して脆弱であるが，裾野の重い $t$ 分布は外れ値に強く，ロバストな推定が可能である．
